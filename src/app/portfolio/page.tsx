...
export default function PortfolioPage() {
  const { holdings, providerErrors, isLoading } = usePortfolio();
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] text-[#172b2c]">
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <Sidebar />
        <main className="relative min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-12 lg:py-9">
          <Header />
            ...
            <>
              ...
              {providerErrors.length > 0 && (
                <div className="mt-4 rounded-xl bg-[#fff5e9] px-4 py-3 text-sm text-[#a9662a]">
                  Some market data could not be refreshed. Unavailable values
                  are shown as N/A.
                </div>
              )}

              <div className="mt-6 overflow-x-auto rounded-xl border border-[#dce7e2] bg-[#fbfdfc] shadow-[0_10px_30px_rgba(36,75,69,0.04)]">
                <table className="w-full min-w-[1300px] text-left">
                  <thead>
                    <tr className="border-b border-[#edf2f0] bg-[#fbfdfc] text-center text-[11px] uppercase tracking-[0.13em] text-[#9aaba7]">
                  ...
                  <tbody className="divide-y divide-[#edf2f0]">
                    {filteredHoldings.map((holding) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right text-[#66807d]">
